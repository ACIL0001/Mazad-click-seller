...
export const authStore = create<IAuthStore>((setValues) => ({
  ...
  set: (auth: typeof initialState) => {
    ...
    // Save the complete auth data to localStorage
    const dataToSave = {
      user: authData.user,
      tokens: authData.tokens
    };
    window.localStorage.setItem('auth', JSON.stringify(dataToSave));
    console.log('🔐 AuthStore.set - Auth data saved to localStorage:', dataToSave);
  },
  clear: () => {
    console.log('🔐 AuthStore.clear called - Clearing auth data');
    setValues((state) => ({ auth: initialState, isLogged: false }));
    window.localStorage.removeItem('auth');
    console.log('🔐 AuthStore.clear - Auth data cleared from localStorage');
  },
  initializeAuth: () => {
    if (!window) return;
    ...
    if (authentication) {
      try {
        ...
        if (parsedAuth && parsedAuth.user && parsedAuth.tokens && 
          ...
          console.log('🔐 Auth initialized from localStorage:', parsedAuth);
        } else {
        ...
        console.log('ℹ️ No auth data in localStorage, using initial state');
